...
extern "C" {
    JNIEXPORT jobject JNICALL Java_io_github_humbleui_skija_impl_BufferUtil__1nGetByteBufferFromPointer
      (JNIEnv *env, jclass, jlong ptr, jint size) {
        return env->NewDirectByteBuffer(jlongToPtr<void*>(ptr), size);
    }

    JNIEXPORT jlong JNICALL Java_io_github_humbleui_skija_impl_BufferUtil__1nGetPointerFromByteBuffer
