...
namespace skija {
    ...
    namespace Color4f {
        ...
        void onLoad(JNIEnv* env) {
            jclass local = env->FindClass("io/github/humbleui/skija/Color4f");
            cls  = static_cast<jclass>(env->NewGlobalRef(local));
    ...
    namespace FontFamilyName {
        ...
        void onLoad(JNIEnv* env) {
            jclass local = env->FindClass("io/github/humbleui/skija/FontFamilyName");
            cls  = static_cast<jclass>(env->NewGlobalRef(local));
    ...
    namespace FontFeature {
        ...
        void onLoad(JNIEnv* env) {
            jclass local = env->FindClass("io/github/humbleui/skija/FontFeature");
            cls  = static_cast<jclass>(env->NewGlobalRef(local));
    ...
    namespace FontMetrics {
        ...
        void onLoad(JNIEnv* env) {
            jclass local = env->FindClass("io/github/humbleui/skija/FontMetrics");
            cls  = static_cast<jclass>(env->NewGlobalRef(local));
    ...
    namespace FontVariation {
        ...
        void onLoad(JNIEnv* env) {
            jclass local = env->FindClass("io/github/humbleui/skija/FontVariation");    
            cls   = static_cast<jclass>(env->NewGlobalRef(local));
    ...
    namespace FontVariationAxis {
        ...
        void onLoad(JNIEnv* env) {
            jclass local = env->FindClass("io/github/humbleui/skija/FontVariationAxis");
            cls  = static_cast<jclass>(env->NewGlobalRef(local));
    ...
    namespace Path {
        ...
        void onLoad(JNIEnv* env) {
            jclass local = env->FindClass("io/github/humbleui/skija/Path");
            cls          = static_cast<jclass>(env->NewGlobalRef(local));
            ctor         = env->GetMethodID(cls, "<init>", "(J)V");
        }
    ...
    namespace PathSegment {
        ...
        void onLoad(JNIEnv* env) {
            jclass local  = env->FindClass("io/github/humbleui/skija/PathSegment");
            cls           = static_cast<jclass>(env->NewGlobalRef(local));
    ...
    namespace PaintFilterCanvas {
        ...
        void onLoad(JNIEnv* env) {
            jclass local = env->FindClass("io/github/humbleui/skija/PaintFilterCanvas");
        ...
        bool onFilter(jobject obj, SkPaint& paint) {
            ...
            return scope.env->CallBooleanMethod(obj, onFilterId, reinterpret_cast<jlong>(&paint));
        }
   ...
   namespace RSXform {
        ...
        void onLoad(JNIEnv* env) {
            jclass local = env->FindClass("io/github/humbleui/skija/RSXform");
            cls  = static_cast<jclass>(env->NewGlobalRef(local));
            ctor = env->GetMethodID(cls, "<init>", "(FFFF)V");
        }
    ...
    namespace impl {
        namespace Native {
            ...
            void onLoad(JNIEnv* env) {
                jclass cls = env->FindClass("io/github/humbleui/skija/impl/Native");
...
std::optional<SkString> skString(JNIEnv* env, jstring s) {
    if (s == nullptr) {
        ...
        jsize len16 = env->GetStringLength(s);
        const jchar* chars = env->GetStringCritical(s, nullptr);
        const uint16_t* src = reinterpret_cast<const uint16_t*>(chars);
        int len8 = SkUTF::UTF16ToUTF8(nullptr, 0, src, len16);
        if (len8 < 0) {
        ...
        SkString res(len8);
        SkUTF::UTF16ToUTF8(res.data(), len8, src, len16);
        env->ReleaseStringCritical(s, chars);
